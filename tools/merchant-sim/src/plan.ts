...
/**
 * Читання плану живе в `packages/chain` (`plan.ts`) — його потребує не лише
 * симулятор мерчанта, а й API (`T035`). Реекспорт тут лишається, щоб CLI і
 * `index.ts` не знали про переїзд.
 */
export {
  NotAPlanError,
  PlanNotFoundError,
